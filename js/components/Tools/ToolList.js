class ToolList {
    constructor(toolService, authService, onEditTool, onDeleteTool, onReserveTool) {
    ...
    async render() {
        ...
        const tools = (await this.toolService.getTools())
            .filter(tool => tool.categoryId != null);
        
        this.allTools = tools;
        this.container.innerHTML = this.createToolsHtml(tools);
        this.setupEventListeners();
            ...
            `;
            document.getElementById('retry-tools')?.addEventListener('click', () => this.render());
        } finally {
    ...
    filterTools(searchTerm = '', categoryId = '') {
        ...
        this.container.innerHTML = this.createToolsHtml(filteredTools);
        this.setupEventListeners();
    }

    createToolsHtml(tools) {
    if (tools.length === 0) {
        return `
    ...
    <div class="card tool-card">
        ...
        <div class="tool-details">
            <p><i class="fas fa-dollar-sign"></i> Costo diario: $${tool.dailyCost?.toFixed(2) || '0.00'}</p>
            <p><i class="fas fa-boxes"></i> Stock: ${tool.stock || 0}</p>
            <p><i class="fas fa-tag"></i> Categoría: ${tool.categoryId ? tool.categoryName : 'Sin categoría'}</p>
            ${tool.supplier ? `<p><i class="fas fa-truck"></i> Proveedor: ${tool.supplier.company || 'Sin proveedor'}</p>` : ''}
        </div>
